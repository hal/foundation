 ...
 * {@link org.jboss.hal.ui.resource.shell.ResourceTabs}. The panel includes a breadcrumb trail for navigation and a copy-to-clipboard
 * button for the resource address.
 * <p>
 ...
 * {@link org.jboss.hal.ui.resource.shell.ResourceTabs}, {@link org.jboss.hal.ui.resource.shell.ResourceList}) for rendering.
 */
        ...
        uic().metadataRepository().lookup(mbn.template, metadata -> {
            ...
            ResourceShell shell = resourceShell(mbn.template, metadata)
                    ...
                    .addHeader(resourceHeader(mbn.template, metadata)
                            .run(rh -> {
                                switch (mbn.type) {
                                    case SINGLETON_FOLDER -> rh.addHeader(span()
                                            .add("Singleton child resources of ")
                                            .add(code().text(mbn.name))
                                            .element());
                                    case FOLDER -> rh.addHeader(span()
                                            .add("Child resources of ")
                                            .add(code().text(mbn.name))
                                            .element());
                                    default -> rh.defaultHeader();
                                }
                            })
                            .defaultDescription());
