...
app.use(express.static(path.join(__dirname, 'target', 'console')));


// 3. J2CL routes
app.get(/.*showcase\.js$/, (req, res) => {
    res.sendFile(path.resolve(__dirname, 'target', 'console', 'console.js'));
});
app.get(/.*bundle\.js$/, (req, res) => {
    const filename = path.basename(req.url);
    res.sendFile(path.resolve(__dirname, 'target', 'console', filename));
});
app.get('/{*pre}/sources/{*post}', (req, res) => {
    const source = req.url.substring(req.url.indexOf('sources/'));
    res.sendFile(path.resolve(__dirname, 'target', 'console', source));
});

// 4. SPA fallback
app.get('/{*any}', (req, res) => {
    res.sendFile(path.resolve(__dirname, 'target', 'console', 'dev.html'));
});
